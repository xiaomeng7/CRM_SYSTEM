...
export default function Configure({products,resumeDraft,actor}:Props){
  ...
  const createProposal=async()=>{if(!customer.name.trim()||!customer.siteAddress.trim()){setMessage("Add the customer name and installation address before creating a Proposal.");return;}setPreparing(true);try{const saved=await saveDraft();if(!saved)throw new Error("The Proposal was not created because the Draft could not be saved.");const customerResponse=await fetch(`/api/drafts/${encodeURIComponent(draftCode)}/customer`,{method:"POST"});const customerResult=await customerResponse.json();if(!customerResponse.ok)throw new Error(customerResult.error||"Unable to save the customer");const ready=await fetch(`/api/drafts/${encodeURIComponent(draftCode)}/ready`,{method:"POST"});const readyResult=await ready.json();if(!ready.ok)throw new Error(readyResult.error||"Unable to mark Draft ready");const response=await fetch(`/api/drafts/${encodeURIComponent(draftCode)}/proposals`,{method:"POST"});const result=await response.json();if(!response.ok)throw new Error(result.error||"Unable to create Proposal");window.location.assign(`/sales/proposals/${encodeURIComponent(result.proposalCode)}`);}catch(error){setMessage(error instanceof Error?error.message:"Unable to create Proposal");setPreparing(false);}};
