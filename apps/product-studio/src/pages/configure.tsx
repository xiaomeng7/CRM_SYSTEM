...
export default function Configure({products,resumeDraft}:Props){
  ...
  const openProposal=async()=>{setPreparing(true);setMessage("Preparing a readable proposal…");try{const result=await requestProjection();const namedLines=result.lines.map((line:{productCode:string})=>{const selectedLine=lines.find(x=>x.code===line.productCode);return {...line,productName:selectedLine?.name||line.productCode};});localStorage.setItem("better-home-proposal-preview-v1",JSON.stringify({...result,lines:namedLines}));window.location.assign("/proposal-preview");}catch(error){setMessage(`Could not prepare proposal: ${error instanceof Error?error.message:"UNKNOWN"}`);setPreparing(false);}};
