...
export default function Configure({products,resumeDraft}:Props){
  ...
  return <><Head><title>Build your Better Home selection</title><meta name="robots" content="noindex"/></Head><main className="cfg-shell">
    ...
    <div className="cfg-layout"><div className="cfg-flow">
      ...
      <section><div className="cfg-step"><b>03</b><div><h2>Experiences</h2><p>Choose the exact room each Experience belongs to.</p></div></div><div className="cfg-grid">{experiences.map(p=>{const targets=compatibleRoomTargets(p);const active=experienceTargets[p.productCode]||[];return <article key={p.productCode} className={`cfg-card cfg-experience ${selectedSet.has(p.productCode)?"is-selected":""}`}><div><span>{p.productCode}</span><h3>{p.canonicalName}</h3><p>{p.customerContent.subtitle||p.hero}</p></div><div className="cfg-room-targets"><small>{targets.length?"APPLY TO":"SELECT A COMPATIBLE COLLECTION FIRST"}</small>{targets.map(target=><button key={target.key} className={active.includes(target.key)?"is-active":""} onClick={()=>toggleExperienceTarget(p,target.key)}>{target.label}</button>)}</div><div className="cfg-card-actions"><strong>{p.activePrice?`${money(p.activePrice.amount)} per room`:"Contact us"}</strong><a href={`/product/${p.productCode}`} target="_blank">View A4</a>{selectedSet.has(p.productCode)&&<button onClick={()=>{setSelected(current=>current.filter(code=>code!==p.productCode));setExperienceTargets(current=>({...current,[p.productCode]:[]}));}}>Remove all</button>}</div></article>;})}</div></section>
