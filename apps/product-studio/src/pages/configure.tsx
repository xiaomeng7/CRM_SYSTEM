...
export default function Configure({products,resumeDraft}:Props){
  ...
  const openProposal=async()=>{const previewWindow=window.open("/proposal-preview","_blank");setPreparing(true);setMessage("Preparing a readable proposal…");try{const result=await requestProjection();const namedLines=result.lines.map((line:{productCode:string})=>{const selectedLine=lines.find(x=>x.code===line.productCode);return {...line,productName:selectedLine?.name||line.productCode};});localStorage.setItem("better-home-proposal-preview-v1",JSON.stringify({...result,lines:namedLines}));if(previewWindow)previewWindow.location.replace("/proposal-preview");else window.location.assign("/proposal-preview");setMessage(`${result.proposalId} is ready to review and print.`);}catch(error){previewWindow?.close();setMessage(`Could not prepare proposal: ${error instanceof Error?error.message:"UNKNOWN"}`);}finally{setPreparing(false);}};
...
export const getServerSideProps:GetServerSideProps<Props>=async context=>{
  ...
  try{let products:Product[];if(configuratorProductCache&&configuratorProductCache.expiresAt>Date.now())products=configuratorProductCache.products;else{const catalog=await os.service.listProducts();products=(await Promise.all(catalog.map((x:{productCode:string})=>os.service.getProduct(x.productCode)))).filter(Boolean);configuratorProductCache={products:JSON.parse(JSON.stringify(products)),expiresAt:Date.now()+5*60*1000};}let resumeDraft=null;const requested=typeof context.query.draft==="string"?context.query.draft:null;if(requested){const {salesStudioService}=require("@bht/product-os/v2");resumeDraft=await salesStudioService.createSalesStudioService(os.prisma).draftDetail(actor,requested);}return {props:{products:JSON.parse(JSON.stringify(products)),resumeDraft:resumeDraft?JSON.parse(JSON.stringify(resumeDraft)):null}};}finally{await os.disconnect();}
};
