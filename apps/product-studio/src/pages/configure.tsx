...
export default function Configure({products,resumeDraft}:Props){
  ...
  const saveDraft=async()=>{setMessage("Saving draft…");localStorage.setItem("better-home-selection-draft-v1",JSON.stringify({schemaVersion:"1.0.0",draftCode,savedAt:new Date().toISOString(),selected:effectiveSelected,quantities,customer}));try{const projection=await requestProjection();const response=await fetch("/api/drafts/save",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({draftCode,projection})});const result=await response.json();if(!response.ok)throw new Error(result.error);setMessage(`${result.draftCode} version ${result.versionNumber} saved in Product OS DEV${result.unchanged?" (unchanged)":""}.`);}catch(error){setMessage(`Saved on this device; DEV server save failed: ${error instanceof Error?error.message:"UNKNOWN"}`);}};
  ...
  return <><Head><title>Build your Better Home selection</title><meta name="robots" content="noindex"/></Head><main className="cfg-shell">
    <header className="cfg-hero"><a href="/product/C-02">BETTER HOME</a><p>PRODUCT CONFIGURATOR</p><h1>Start with the way you want to live.</h1><span>Choose a foundation, add the rooms that matter, then extend only the experiences already in your home.</span></header>
...
export const getServerSideProps:GetServerSideProps<Props>=async context=>{
  ...
  try{const catalog=await os.service.listProducts();const products=(await Promise.all(catalog.map((x:{productCode:string})=>os.service.getProduct(x.productCode)))).filter(Boolean);let resumeDraft=null;const requested=typeof context.query.draft==="string"?context.query.draft:null;if(requested){const {salesStudioService}=require("@bht/product-os/v2");resumeDraft=await salesStudioService.createSalesStudioService(os.prisma).draftDetail(actor,requested);}return {props:{products:JSON.parse(JSON.stringify(products)),resumeDraft:resumeDraft?JSON.parse(JSON.stringify(resumeDraft)):null}};}finally{await os.disconnect();}
};
