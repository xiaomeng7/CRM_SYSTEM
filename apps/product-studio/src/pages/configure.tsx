...
export default function Configure({products,resumeDraft}:Props){
  ...
  const openProposal=()=>{setPreparing(true);const savedAt=new Date().toISOString();const proposal={proposalId:draftCode,status:"INDICATIVE PREVIEW",generatedAt:savedAt,customer,currencyCode:"AUD",taxBasis:"GST_INCLUSIVE",lines:lines.map(line=>({productCode:line.code,productName:line.name,quantity:line.qty,unitPrice:line.amount,lineTotal:line.amount*line.qty})),total,commercialNotice:"Indicative selection only. Final scope, site conditions and pricing are confirmed before contract."};localStorage.setItem("better-home-selection-draft-v1",JSON.stringify({schemaVersion:"1.0.0",draftCode,savedAt,selected:effectiveSelected,quantities,customer}));localStorage.setItem("better-home-proposal-preview-v1",JSON.stringify(proposal));window.location.assign("/proposal-preview");};
