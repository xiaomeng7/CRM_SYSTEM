...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  const base=String(process.env.BETTER_HOME_CRM_INTERNAL_URL||"").replace(/\/$/,""),secret=String(process.env.BETTER_HOME_HANDOFF_INTERNAL_SECRET||""),opportunityId=String(req.body?.opportunityId||""),assetId=String(req.body?.assetId||"");
  ...
  try{
    const liveResponse=await fetch(`${base}/api/internal/better-home-handoffs/crm-contexts/${encodeURIComponent(opportunityId)}?assetId=${encodeURIComponent(assetId)}`,{headers:{"x-bh-handoff-secret":secret},signal:AbortSignal.timeout(15000)}),live=await liveResponse.json();
