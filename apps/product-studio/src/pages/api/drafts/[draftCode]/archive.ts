...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  try{
    const result=await salesDraftService.createSalesDraftService(os.prisma).archiveDraft({draftCode:String(req.query.draftCode||""),actor});
    return res.status(200).json(result);
  }catch(error){
    return res.status(400).json({error:error instanceof Error?error.message:"DRAFT_DELETE_FAILED"});
