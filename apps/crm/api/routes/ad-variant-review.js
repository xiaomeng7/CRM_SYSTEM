/**
 * Ad variant review / approve / reject (API only).
 *
 * GET  /api/ad-variants/review
 * POST /api/ad-variants/review
 * POST /api/ad-variants/bulk-review
 */
...
router.get('/review', async (req, res) => {
  try {
    ...
    if (req.query.channel) {
      params.push(String(req.query.channel).trim());
    ...
    const r = await pool.query(
      `SELECT id, headline, body_text, call_to_action, status, channel, product_focus, campaign_id, created_at
       FROM ad_variants
       ${where}
    ...
    console.error('[ad-variant-review]', err);
...
router.post('/review', async (req, res) => {
  try {
    ...
    if (!r.rows[0]) {
      return res.status(404).json({ ok: false, error: 'ad variant not found' });
    }
