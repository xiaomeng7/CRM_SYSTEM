...
function createSalesLifecycleService(prisma){
  ...
  async function recordProposalDelivery(actor,proposalCodeValue,input={}){
    const a=assertCan(actor,"PROPOSAL_SEND_OWN"),code=normalizeProposalCode(proposalCodeValue),channel=String(input.channel||"").trim().toUpperCase(),recipient=String(input.recipient||"").trim(),evidenceReference=String(input.evidenceReference||"").trim();
