...
test("sales can write and send their own approved work but cannot approve",()=>{const a={userId:"u1",role:"SALES"};assert.equal(canAccessDraft(a,"u1","WRITE"),true);assert.equal(canAccessDraft(a,"u2","WRITE"),false);assert.equal(can(a,"PROPOSAL_SEND_OWN"),true);assert.equal(can(a,"PROPOSAL_APPROVE"),false);});
